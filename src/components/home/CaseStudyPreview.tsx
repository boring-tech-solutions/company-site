...
const caseStudies = [{
  ...
  results: ["70% reduction in manual data processing time", "45% improvement in decision-making speed", "24/7 automated customer support handling", "Real-time predictive insights for business planning"],
  ...
  results: ["300% increase in volunteer coordination efficiency", "Unified data across 15+ community chapters", "Real-time impact measurement and reporting", "Mobile-first design for field organizers"],
  ...
  results: ["Improved learner engagement through interactive quizzes", "Clear progress tracking to support consistent study habits", "Increased learner confidence through immediate feedback", "Accessible learning experience for diverse newcomer communities", "Scalable platform suitable for workshops, classrooms, and independent study"],
  ...
  return <section id="case-studies" className="relative py-[75px]">
      ...
      <Dialog open={!!selectedStudy} onOpenChange={() => setSelectedStudy(null)}>
        <DialogContent className="max-w-3xl border-none p-0 overflow-hidden">
          {selectedStudy && <div className={`relative min-h-[600px] ${selectedStudy.bgColor}`}>
              {/* Content */}
              <div className={`relative z-10 p-8 ${selectedStudy.textColor}`}>
                ...
                <div className="mb-6">
                  ...
                  <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {selectedStudy.results.map((result, idx) => <li key={idx} className={`flex items-start gap-2 text-sm ${selectedStudy.mutedColor}`}>
                ...
                <div className="flex flex-wrap gap-6 mb-6 pt-4 border-t border-white/20">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedStudy.technologies.map((tech, idx) => <span key={idx} className={`text-xs px-2 py-1 rounded-full ${selectedStudy.textColor === 'text-white' ? 'bg-white/20' : 'bg-charcoal-deep/20'}`}>
                          {tech}
                        </span>)}
                    </div>
                  </div>
